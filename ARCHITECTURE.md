...
### D.0 Stack decision (Phase 1)

Node.js + TypeScript + Fastify for the API, chosen for a lightweight plugin architecture that
maps cleanly onto the `modules/`/`shared/` boundaries below, and because it keeps deployment
options open (including Cloudflare Workers, if that ends up being the answer once Cloudflare's
role is decided — see CLAUDE.md's Infrastructure status). Database access goes through
`@supabase/supabase-js` with the service-role key, server-side only, never sent to the client.
