...
**Rebrand note:** "Nomad" is retired as the user-facing product name; the product going forward is **REKT**. This is a branding change, not an architecture change — the existing codebase, database, and UI are referred to below as "Legacy Nomad V0," and the target system is "REKT." Nothing in the technical plan changes because of the rename; only user-facing strings, the docs in §H, and eventually the deployment/domain need to change. Rename this file's references as convenient — the underlying `nomad.html`/Supabase project can keep its internal name until Phase 1 touches it.
...
None of this is a knock on the prototype — it clearly works. But it means **Phase 1 cannot just be "add a backend for the new features."** It has to include closing the live authorization gaps in the current system, because those gaps get worse, not better, as more users sign up.
...
### C.1 Findings, ranked by severity

**🔴 Critical — Likely cross-tenant data exposure and privilege escalation (live in production today)**

The only RLS policy present in the codebase is:
...
- **Immediate remediation checklist** (do this regardless of the rest of the roadmap):
  1. Pull the actual deployed RLS policies from the Supabase dashboard and compare against what's assumed above.
  2. Lock `server_memberships`, `trades`, `users`, `servers` writes to owner-scoped or admin-scoped policies.
  3. Rotate the `seed` values and stop deriving join codes client-side.
  4. Force-reset any admin passwords stored via the unsalted hash and remove that path in favor of Discord OAuth only.
...
1. **Phase 0 — Audit (this document).** Also: pull real deployed RLS policies and compare to the assumptions above; patch the critical/high findings in the *live* database this week, independent of the rebuild.
...
Before any Phase 1 feature work: verify the actual deployed Supabase RLS policies against the findings in §C.1, since the code strongly implies a live, exploitable authorization gap in the current production data. Everything else in this plan can wait a sprint; that can't.
